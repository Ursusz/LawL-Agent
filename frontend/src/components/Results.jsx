                    ...
                    <div className="p-6 space-y-4">
                      ...
                      {item.law_details && Object.keys(item.law_details).length > 0 ? (
                        <div className="space-y-3">
                          {Object.entries(item.law_details)
                            .sort(([, a], [, b]) => {
                              // Sort by BM25 score of first article (highest first)
                              const scoreA = a.relevant_articles?.[0]?.score ?? -Infinity;
                              const scoreB = b.relevant_articles?.[0]?.score ?? -Infinity;
                              return scoreB - scoreA;
                            })
                            .map(([lawRef, law_details]) => (
